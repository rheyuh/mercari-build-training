...
# overriding production db with test db
...
def test_add_item_e2e(args,want_status_code,db_connection):
    ...
    if want_status_code >= 400:
        #verify test failure mode
        response_data = response.json()
        assert response_data["detail"] == "name is required"
        return
    ...
    # makes each row its own dictionary
    for row in db_items:
        assert dict(row)["name"] != None